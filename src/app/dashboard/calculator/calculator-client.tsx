...
import { calculatePrice } from "@/lib/db/actions/calculator-actions";
...
export default function CalculatorClient({ initialTests }: { initialTests: TestRow[] }) {
  ...
  const filteredTests = useMemo(() => {
    ...
    return initialTests.filter((t) => t.testId.toLowerCase().includes(q) || t.name.toLowerCase().includes(q));
  }, [initialTests, search]);

  const selectedTests = useMemo(
    () => initialTests.filter((t) => selectedIds.has(t.id)),
    [initialTests, selectedIds]
  );
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-xs font-mono text-text-muted">{test.testId}</span>
                        </div>
                      </div>
                      <span className="font-mono text-sm text-text-secondary">${num(test.listPrice).toFixed(2)}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <div className="col-span-7 space-y-6 animate-fade-in stagger-2 opacity-0">
            ...
            <div className="bg-surface-raised rounded-xl border border-border">
              ...
              <div className="p-5">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {selectedTests.map((test) => (
                      <span key={test.id} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface text-sm font-body text-text-primary border border-border">
                        <span className="font-mono text-xs text-accent">{test.testId}</span>
                        {test.name}
                ...
                >
                  {calculating ? "Calculating..." : "Calculate Panel Price"}
                </button>
              </div>
            </div>
              ...
              <div className="bg-surface-raised rounded-xl border border-border animate-scale-in">
                ...
                <div className="divide-y divide-border">
                  {result.tests.map((detail) => (
                    <div key={detail.testId} className="px-5 py-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-sm font-body text-text-primary">{detail.testName}</span>
