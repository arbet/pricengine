...
export default function AdminPage() {
  ...
  // Add user form (inside org accordion)
  ...
  const handleRemoveUser = (userId: string, orgId: string) => {
    ...
    setUsers(updatedUsers);
    const newCount = updatedUsers.filter((u) => u.orgId === orgId).length;
    setOrgs(orgs.map((o) => o.id === orgId ? { ...o, usersCount: newCount } : o));
  };
  ...
  return (
    <div>
      ...
      <div className="p-8">
        ...
        {/* Org accordion list */}
        <div className="space-y-2 animate-fade-in stagger-2 opacity-0">
          {orgs.map((org) => {
            ...
            return (
              <div
                key={org.id}
                className="bg-surface-raised rounded-xl border border-border overflow-hidden transition-all"
              >
                {/* Accordion header */}
                <button
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right hidden sm:block">
                      <div className="text-xs font-body text-text-secondary">
                        {members.length} user{members.length !== 1 ? "s" : ""} &middot; {org.testsCount} tests
                      </div>
                      <div className="text-xs font-body text-text-muted mt-0.5">Since {org.createdAt}</div>
                    </div>
                    <svg
                      width="16"
                      height="16"
                ...
                {isExpanded && (
                  <div className="border-t border-border">
                    ...
                    <div className="px-5 pt-4 pb-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
                      <div>
                        <p className="text-[11px] font-body font-medium text-text-muted uppercase tracking-wider">Email</p>
                        <p className="text-sm font-body text-text-secondary mt-0.5">{org.contactEmail}</p>
                      </div>
                      <div>
                        <p className="text-[11px] font-body font-medium text-text-muted uppercase tracking-wider">Phone</p>
                        <p className="text-sm font-body text-text-secondary mt-0.5">{org.phone}</p>
                      </div>
                      <div>
                        <p className="text-[11px] font-body font-medium text-text-muted uppercase tracking-wider">Code</p>
                        <p className="text-sm font-mono text-accent mt-0.5">{org.code}</p>
                      </div>
                      <div>
                        <p className="text-[11px] font-body font-medium text-text-muted uppercase tracking-wider">Address</p>
                        <p className="text-sm font-body text-text-secondary mt-0.5">{org.address}</p>
                      </div>
                    </div>
                    ...
                    <div className="px-5 pb-5 border-t border-border pt-4">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-display font-semibold text-sm text-text-primary">
                          Users &amp; Roles
                          <span className="ml-2 font-mono text-xs text-text-muted font-normal">({members.length})</span>
                        </h4>
                        <button
                      ...
                      {members.length > 0 ? (
                        <div className="border border-border rounded-xl overflow-hidden">
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {members.map((u, idx) => (
                                <tr key={u.id} className={`border-b border-border last:border-0 ${idx % 2 === 0 ? "" : "bg-surface/50"}`}>
                                  ...
                                  <td className="px-4 py-2.5 text-right">
                                    <div className="flex items-center justify-end gap-1">
                                      <button
                                        ...
                                        title="Edit user"
                                        className="p-1.5 text-text-muted hover:text-accent hover:bg-accent-muted rounded-lg transition-colors"
                                      >
                                      ...
                                      <button
                                        onClick={() => handleRemoveUser(u.id, org.id)}
                                        title="Remove user"
                        ...
                        <div className="py-6 text-center text-sm text-text-muted font-body border border-dashed border-border rounded-xl">
